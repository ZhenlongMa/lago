...
OBJ_DIR = "/work/mazhenlong/CX-test/th_perftest_script"
...
def start_test(qp_num, msg_sz):
    SVR_CMD1 = TEST_TYPE + " -p 12331 -d " + SVR1_DEV + " -i 1 -l " + str(WQE_NUM) + " -m " + str(MTU) + " -c RC -q " + str(qp_num) + " -F -s " + str(msg_sz) + " --run_infinitely"
    SVR_CMD2 = TEST_TYPE + " -p 12332 -d " + SVR2_DEV + " -i 1 -l " + str(WQE_NUM) + " -m " + str(MTU) + " -c RC -q " + str(qp_num) + " -F -s " + str(msg_sz) + " --run_infinitely"
    SVR_CMD3 = TEST_TYPE + " -p 12333 -d " + SVR3_DEV + " -i 1 -l " + str(WQE_NUM) + " -m " + str(MTU) + " -c RC -q " + str(qp_num) + " -F -s " + str(msg_sz) + " --run_infinitely"
    SVR_CMD4 = TEST_TYPE + " -p 12334 -d " + SVR4_DEV + " -i 1 -l " + str(WQE_NUM) + " -m " + str(MTU) + " -c RC -q " + str(qp_num) + " -F -s " + str(msg_sz) + " --run_infinitely"
    CLT1_CMD = TEST_TYPE + " -p 12331 -d " + CLT1_DEV + " -i 1 -l " + str(WQE_NUM) + " -m " + str(MTU) + " -c RC -q " + str(qp_num) + " -F -s " + str(msg_sz) + " --run_infinitely " + SVR1
    CLT2_CMD = TEST_TYPE + " -p 12332 -d " + CLT2_DEV + " -i 1 -l " + str(WQE_NUM) + " -m " + str(MTU) + " -c RC -q " + str(qp_num) + " -F -s " + str(msg_sz) + " --run_infinitely " + SVR2
    CLT3_CMD = TEST_TYPE + " -p 12333 -d " + CLT3_DEV + " -i 1 -l " + str(WQE_NUM) + " -m " + str(MTU) + " -c RC -q " + str(qp_num) + " -F -s " + str(msg_sz) + " --run_infinitely " + SVR3
    CLT4_CMD = TEST_TYPE + " -p 12334 -d " + CLT4_DEV + " -i 1 -l " + str(WQE_NUM) + " -m " + str(MTU) + " -c RC -q " + str(qp_num) + " -F -s " + str(msg_sz) + " --run_infinitely " + SVR4
    cmd_list = [
...
def stop(machine_list):
    for node in machine_list:
        cmd = "ssh root@" + node + " 'netstat -t -p > " + OBJ_DIR + "/tmp.log'"
        print(cmd)
        rtn = os.system(cmd)
        if rtn != 0:
        ...
        with open(OBJ_DIR + "/tmp.log", "r", encoding ="utf-8") as f:
            for line in f.readlines():
                ...
                print(res)
                if res != []:
                    print("ssh root@" + node + " 'kill -9 " + res[0] + "'")
                    os.system("ssh root@" + node + " 'kill -9 " + res[0] + "'")
...
def main():
    ...
    # qp_num_list = [4, 16, 64, 256, 1024, 4096]
